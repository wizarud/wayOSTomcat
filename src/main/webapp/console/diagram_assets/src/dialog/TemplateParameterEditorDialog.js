limz_TemplateParameterEditorDialog = Class.extend({
    ...
    _getSuggestions: function (callback) {
        app.getBackend().getPrerequisitVariables(this.parameterName, $.proxy(function(variables){
    ...
    _onOk: function (value) {
        alert("inherit classes must implement the Method 'limz_TemplateEditorDialog#_onOk'");
    },
    ...
    _updateEditor: function () {
        try {
            ...
            //
            var menu = $("#suggest");
            if (val.trim().length===0 || !this.suggestions.includes(val.trim())) {
                var showList = this.suggestions;
                menu.html("");
                if (showList.length > 0) {
                    ...
                    $.each(showList, $.proxy(function (i, e) {
                        ...
                        item.on("click", $.proxy(function () {
                            ...
                            menu.hide();
                            this.editor.focus();
                            this.editor.caretPosition(e.length);
                            this._updateEditor();
                        }, this));
                        menu.append(item);
                    }, this));
